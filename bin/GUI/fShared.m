...
if size(Objects{idx}.data{k},2)==1
    Molecule(nMol).Results(nData,10:11) = Objects{idx}.data{k}';                
    Molecule(nMol).Results(nData,12) = single(mod(Objects{idx}.orientation(1,k),2*pi));                
    Molecule(nMol).Type = 'stretched';
elseif size(Objects{idx}.data{k},2)==3
    Molecule(nMol).Results(nData,10:12) = Objects{idx}.data{k}(1,:);                
...
if ~isempty(Filament(nFil).Results) && isempty(nData)
    ...
    if ~isempty(f)
        ...
        Filament(nMol).TrackingResults(f+1:n+1)=Filament(nMol).TrackingResults(f:n);
    else
...
Filament(nMol).TrackingResults{nData} = [];
if nData > 1
