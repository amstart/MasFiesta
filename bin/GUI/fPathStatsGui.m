        ...
        if isempty(PathStats(n).PathData)
            if size(PathStats(n).Results,1)>9&&~strcmp(button,'Filament')
                if strcmp(button,'Fit')
                    ...
                    [param2,resnorm2] = PathFitCurved(PathStats(n).Results(:,3:5),param1,3); 
                    [param3,resnorm3] = PathFitCurved(PathStats(n).Results(:,3:5),param1,4);
...
if gcbo==hPathsStatsGui.lAll||gcbo==hPathsStatsGui.bReset||gcbo==hPathsStatsGui.bDisregard
    ...
    for i = n
        if gcbo==hPathsStatsGui.bAuto
            ...
            [param2,resnorm2] = PathFitCurved(PathStats(i).Results(:,3:5),param1,3); 
            [param3,resnorm3] = PathFitCurved(PathStats(i).Results(:,3:5),param1,4);
            if all(resnorm1*1.1<[resnorm2 resnorm3])
            ...
            elseif get(hPathsStatsGui.rPoly2,'Value')==1
                 [param1,~] = PathFitLinear(PathStats(i).Results(:,3:5));
                 [param2,resnorm] = PathFitCurved(PathStats(i).Results(:,3:5),param1,3); 
                 if resnorm<1e+100
                     Path = EvalCurvedPath(param2,PathStats(i).Results(:,3:5));
                     PathStats(i).AverageDis = -2;
                 else
            ...
            elseif get(hPathsStatsGui.rPoly3,'Value')==1
                [param1,~] = PathFitLinear(PathStats(i).Results(:,3:5));
                [param3,resnorm] = PathFitCurved(PathStats(i).Results(:,3:5),param1,4); 
                if resnorm<1e+100
                    Path = EvalCurvedPath(param3,PathStats(i).Results(:,3:5));
                    PathStats(i).AverageDis = -3;
                else
...
set(hPathsStatsGui.fig,'CurrentAxes',hPathsStatsGui.aPlotXYZ);
PathStats=getappdata(hPathsStatsGui.fig,'PathStats');
...
if isempty(ZPlot)
    ...
    line(XPlotPath/scale,YPlotPath/scale,'Color','green','LineStyle','-','Marker','none');
...
function [path,resnorm]=PathFitCurved(XYZ,param0,knots)
XYZ = double(XYZ);
flip = 0;
if max(XYZ(:,2))-min(XYZ(:,2))>max(XYZ(:,1))-min(XYZ(:,1))
    param0(:,1:2) = fliplr(param0(:,1:2));
    XYZ(:,1:2)=fliplr(XYZ(:,1:2));
    flip = 1;
end
if XYZ(1,1)>XYZ(end,1)
    param0 = flipud(param0);
end
