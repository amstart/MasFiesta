...
iperm = 172;
for i = 1:length(measuredLines)
signal = Image(round(min(measuredLines(i).Y)-4:max(measuredLines(i).Y)),round(min(measuredLines(i).X):max(measuredLines(i).X)));
itracemat = double(signal(5:end,:))./iperm;
bg = mean(signal(1:4,:))./iperm;
...
for iframe = find(select)'
    itrace = (itracemat(iframe,:)-bg)./ 64.54;%157
